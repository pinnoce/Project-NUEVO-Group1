...
The shipped `main.py` runs this sequence on `BTN_1` (with `BTN_2` cancelling to `IDLE` at any stage). Static check: every `state = 'X'` / `_after_pause = 'X'` target resolves to a defined `elif state == 'X'` (91 states, no orphans).
...
4. **Burger sub-FSM** — 4 stops `BUR_0..3` (patty / left-bun / right-bun / stack). Per stop: `BUR_n_TURN` (per-stop shelf offset) → `BUR_n_ALIGN` (forward-wall square) → `BUR_n_APPROACH` (drive until `closest_forward_obstacle_mm ≤ standoff`) → manipulate (lower → grip/release → raise) → retreat.
