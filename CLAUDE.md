...
The shipped `main.py` runs this sequence on `BTN_1` (with `BTN_2` cancelling to `IDLE` at any stage). Static check: every `state = 'X'` / `_after_pause = 'X'` target resolves to a defined `elif state == 'X'` (92 states, no orphans).
...
3. `MOV1_DRIVE` — drive forward to the patty shelf.
4. **Burger sub-FSM** — 4 stops `BUR_0..3` (patty / left-bun / right-bun / stack). Per stop: `BUR_n_TURN` (per-stop shelf offset) → `BUR_n_ALIGN` (forward-wall square) → `BUR_n_APPROACH` (drive until `closest_forward_obstacle_mm ≤ standoff`) → manipulate (lower → grip/release → raise) → retreat. Inter-stop travel turns are **relative** (`INTER_TURN_DEG`); the LB→RB turn (`INTER_TURN_DEG[1] = -120°`) slips on the floor, so a lidar parallel re-square `BUR_TRAVEL_12_ALIGN` (left wall, FOV `BUR_TRAVEL_12_ALIGN_FOV`) runs between that turn and its 309 mm drive to keep the leg tracking straight along the shelf.
