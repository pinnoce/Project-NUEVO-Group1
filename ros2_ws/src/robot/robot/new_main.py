...
WALL_ALIGN_MAX_ANGULAR_RAD_S = 0.6
...
# ---------------------------------------------------------------------------
MOV1_DISTANCE_MM   = 1130.0    # forward distance to reach patty position
...
MOV2_ALIGN_LEFT_FOV  = 20.0   # FOV for all left-wall parallel aligns
MOV2_ALIGN_RIGHT_FOV = 20.0   # FOV for all right-wall parallel aligns
...
MOV3_TURN_DEG           = -90.0  # right turn after wall approach
MOV3_TURN_MAX_ANGULAR_RAD_S = 1.0
MOV3_TURN_TOLERANCE_DEG = 3.0
MOV3_ALIGN_PARA_FOV     = 25.0   # FOV for parallel left-wall alignment
MOV3_DRIVE_MM           = 500.0
MOV3_VELOCITY_MM_S      = 120.0
MOV3_TOLERANCE_MM       = 20.0
MOV3_DRIVE_TIMEOUT_S    = 30.0
MOV3_POST_DRIVE_PAUSE_S = 1.0
...
MOV4_DRIVE_FEMALE_MM    = 2000.0
MOV4_DRIVE_MALE_MM      = 2230.0

# ---------------------------------------------------------------------------
# Burger drop-off
# ---------------------------------------------------------------------------
DROP_SHELF_TURN_DEG     = 92.0    # CCW (left) from current heading to face shelf
DROP_SHELF_ALIGN_FOV    = 20.0
DROP_SHELF_STANDOFF_MM  = 300.0
...
# Stop sign
# ---------------------------------------------------------------------------
MIN_STOP_SIGN_CONFIDENCE = 0.95   # YOLO detection confidence threshold (adjustable)
STOP_SIGN_VEL_MM_S      = 80.0
STOP_SIGN_WAIT_S        = 3.0
STOP_SIGN_FINAL_MM      = 300.0   # drive this far after the stop
...
def reset_mission(robot: Robot) -> None:
    global _LIFT_LOGICAL_STEPS, _detected_gender, _lift_primed
    ...
    while True:
        ...
        # ===================================================================
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                if corr is None:
                    if now - _align_started_at >= WALL_ALIGN_TIMEOUT_S:
                        _align_started_at = 0.0; state = 'MOV2_DRIVE_2'
                elif abs(corr) <= WALL_ALIGN_TOLERANCE_DEG or _align_iters >= WALL_ALIGN_MAX_ITERS:
                    _align_started_at = 0.0; state = 'MOV2_DRIVE_2'
                else:
                    _align_iters += 1; motion_handle = _start_turn_to(robot, robot.get_pose()[2] + corr)

        elif state == 'MOV2_DRIVE_2':
            _align_started_at = 0.0
            motion_handle = _start_drive(robot, MOV2_DRIVE_2_MM, MOV2_VELOCITY_MM_S,
                                         MOV2_TOLERANCE_MM, MOV2_DRIVE_TIMEOUT_S)
            state = 'MOV2_DRIVE_2_WAIT'

        elif state == 'MOV2_DRIVE_2_WAIT':
            if robot.was_button_pressed(Button.BTN_2):
                abort_to_idle(robot, motion_handle, 'cancelled MOV2 D2'); motion_handle = None; state = 'IDLE'
            elif motion_handle is not None and motion_handle.is_finished():
                motion_handle = None
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                if corr is None:
                    if now - _align_started_at >= WALL_ALIGN_TIMEOUT_S:
                        _align_started_at = 0.0; state = 'MOV2_DRIVE_3'
                elif abs(corr) <= WALL_ALIGN_TOLERANCE_DEG or _align_iters >= WALL_ALIGN_MAX_ITERS:
                    _align_started_at = 0.0; state = 'MOV2_DRIVE_3'
                else:
                    _align_iters += 1; motion_handle = _start_turn_to(robot, robot.get_pose()[2] + corr)

        elif state == 'MOV2_DRIVE_3':
            _align_started_at = 0.0
            motion_handle = _start_drive(robot, MOV2_DRIVE_3_MM, MOV2_VELOCITY_MM_S,
                                         MOV2_TOLERANCE_MM, MOV2_DRIVE_TIMEOUT_S)
            state = 'MOV2_DRIVE_3_WAIT'

        elif state == 'MOV2_DRIVE_3_WAIT':
            if robot.was_button_pressed(Button.BTN_2):
                abort_to_idle(robot, motion_handle, 'cancelled MOV2 D3'); motion_handle = None; state = 'IDLE'
            elif motion_handle is not None and motion_handle.is_finished():
                motion_handle = None
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                if corr is None:
                    if now - _align_started_at >= WALL_ALIGN_TIMEOUT_S:
                        _align_started_at = 0.0; state = 'MOV2_DRIVE_4'
                elif abs(corr) <= WALL_ALIGN_TOLERANCE_DEG or _align_iters >= WALL_ALIGN_MAX_ITERS:
                    _align_started_at = 0.0; state = 'MOV2_DRIVE_4'
                else:
                    _align_iters += 1; motion_handle = _start_turn_to(robot, robot.get_pose()[2] + corr)

        elif state == 'MOV2_DRIVE_4':
            _align_started_at = 0.0
            motion_handle = _start_drive(robot, MOV2_DRIVE_4_MM, MOV2_VELOCITY_MM_S,
                                         MOV2_TOLERANCE_MM, MOV2_DRIVE_TIMEOUT_S)
            state = 'MOV2_DRIVE_4_WAIT'

        elif state == 'MOV2_DRIVE_4_WAIT':
            if robot.was_button_pressed(Button.BTN_2):
                abort_to_idle(robot, motion_handle, 'cancelled MOV2 D4'); motion_handle = None; state = 'IDLE'
            elif motion_handle is not None and motion_handle.is_finished():
                motion_handle = None
        ...
        # ===================================================================
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                motion_handle = None
                robot.stop()
                _pause_until = now + LAPF_PAUSE_S; _after_pause = 'MOV3_ALIGN_PERP'; state = 'PAUSING'
        ...
        # ===================================================================
            ...
            if robot.was_button_pressed(Button.BTN_2):
                ...
                motion_handle = None; state = 'STOP_DRIVE'
