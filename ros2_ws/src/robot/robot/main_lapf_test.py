...
# ---------------------------------------------------------------------------
# TEST HARNESS (this file only) — start at the LAPF leg
# ---------------------------------------------------------------------------
# This is a trimmed-behaviour copy of main.py used to tune the POST-LAPF section
# in isolation. On BTN_1 the FSM skips traffic-light / MOV1 / burger / MOV2 /
# LAPF and jumps straight to MOV3_ALIGN_PERP (then MOV3 / MOV4 / gender /
# drop-off / finish run exactly as in the real mission). MOV3+ alignment is
# lidar-relative (no absolute world goal), so place the robot where it would be
# when LAPF finishes — at the start of the MOV3 perpendicular-wall approach.
    ...
    while True:
        ...
        # ===================================================================
            ...
            if robot.was_button_pressed(Button.BTN_1):
                ...
                # TEST HARNESS: skip everything up to and including the LAPF leg
                # and jump straight to the post-LAPF sequence (MOV3 onward) so
                # that section can be tuned in isolation. MOV3/MOV4 alignment is
                # lidar-relative (no absolute world goal), so the reset odometry
                # heading is fine — just place the robot where it would be when
                # LAPF finishes (start of the MOV3 perpendicular-wall approach).
                if TEST_SIMULATE_CARRY:
                    simulate_carry(robot)
                print('[TEST] jumping to post-LAPF — MOV3_ALIGN_PERP')
                state = 'MOV3_ALIGN_PERP'
